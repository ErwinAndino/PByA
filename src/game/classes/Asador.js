...
export class Asador extends KitchenBox {
    constructor(scene, x, y, size, frame) {
        const textureKey = "grill";
        const coalFrame = 0;
        const textureCoal = scene.add.sprite(x, y, "embers", coalFrame); // crea las embers antes que el grill para que se rendericen atras de este
        ...
        this.hasCoal = false;
        this.coalIcon = scene.add.image(x, y - 10, "iconHotCoal");
        this.coalIcon.setDepth(10);
        this.coalIcon.setVisible(false);
        ...
        this.emitterHumo2 = this.scene.add.particles(x, y, 'particleSmoke02', { // humo chico
            frame: [0, 1, 2, 3],
            speedX: { min: -10, max: 10 },
            speedY: { min: -20, max: -40 },
            lifespan: 1500,
